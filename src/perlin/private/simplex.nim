...
proc getCornerOffset(
    point: Point2D[float], ijk: tuple[i, j: int], multiplier: float
): Point2D[float] {.inline.} =
  ## Calculates the offset for various corners
  (x: point.x - float(ijk.i) + multiplier, y: point.y - float(ijk.j) + multiplier)
...
proc contribution(
    self: Noise, point: Point3D[float], unit: Point3D[int], ijk: tuple[i, j, k: int]
): float {.inline.} =
...
proc contribution(
    self: Noise, point: Point2D[float], unit: Point2D[int], ijk: tuple[i, j: int]
): float {.inline.} =
  ## Noise contributions from a corners
