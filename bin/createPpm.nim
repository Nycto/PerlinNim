...
type PpmImage = object ## Creates a PPM file
  file: File
  width, height: int
...
proc newPPM(filename: string, width, height: int): PpmImage =
  ...
  var image = PpmImage(file: open(filename, fmwrite), width: width, height: height)
...
for point in image.pixels:
  let shade =
    int(255 * noiseConf.get(noiseType, float(point.x) / zoom, float(point.y) / zoom))
